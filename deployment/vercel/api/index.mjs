...
async function backend() {
  ...
  const allowedHosts=['api.openai.com','api.tavily.com','api.elevenlabs.io','jcynpdtvuqrppkkgegbt.supabase.co'];
  ...
  let response;
  try { response=await fetch(origin+'/__bootstrap',{method:'POST',headers:{'content-type':'application/json','x-twodb-gateway':process.env.TWO_DB_CLOUD_GATEWAY_KEY},body:JSON.stringify({apiKey:process.env.TWO_DB_OPENAI_API_KEY||'',tavilyApiKey:process.env.TWO_DB_TAVILY_API_KEY||'',model:process.env.TWO_DB_AGENT_MODEL||'',engineerKey:process.env.TWO_DB_CLOUD_ENGINEER_KEY,supabaseDbUrl:process.env.SUPABASE_DB_URL||'',supabaseUrl:process.env.SUPABASE_URL||'',supabasePublishableKey:process.env.SUPABASE_PUBLISHABLE_KEY||'',elevenLabsApiKey:process.env.ELEVENLABS_API_KEY||'',elevenLabsAgentId:process.env.ELEVENLABS_AGENT_ID||''}),signal:AbortSignal.timeout(120000)}); }
