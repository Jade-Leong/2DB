...
function ticketStatus(t) {
  ...
  if (p?.state === "Approved") return {label:"Approved", tone:"good"};
  ...
  if (p?.state === "Authorized for verification") return {label:"Verifying", tone:"active"};
...
function stageName(state) {
  return ({ "Rejected": "Denied", "Approved": "Approved", "Model response": "Investigating", "Live Agent 2 running": "Verifying", "Verification running": "Running checks", "Ready for Agent 2": "Ready for Agent 2", "Awaiting engineer approval": "Ready to verify", "Authorized for verification": "Ready to verify", "Researching with Tavily": "Researching documentation" })[state] || state || "Not started";
}
...
function reviewedByAgent2(p) {
  const run = p?.runs?.[0];
  const authorization = currentApproval(p);
  return run?.verification_mode === "live-agent-2" && run.finished_at && run.approval_id === authorization?.id && run.candidate_revision === p.candidate_revision && run.base_revision === p.base_revision && run.requirements_hash === p.requirements_hash && run.harness_hash === p.harness_hash && run.revision_number === p.revision_number;
}
...
function workspaceView(t) {
  ...
  const visibleTickets = inboxReadFilter === "unread" ? tickets.filter(x => !readTicketIds.has(x.id)) : inboxReadFilter === "approved" ? tickets.filter(x => ticketStatus(x).label === "Approved") : tickets;
  return `<div class="workbench" data-pane="${mobileWorkspacePane}"><div class="mobile-workspace-nav" aria-label="Workspace views"><button class="text" data-action="show-inbox" aria-pressed="${mobileWorkspacePane === "inbox"}">Inbox · ${tickets.length}</button><button class="text" data-action="show-ticket" ${!t ? "disabled" : ""} aria-pressed="${mobileWorkspacePane === "ticket"}">Selected ticket</button></div><div class="review-layout"><aside class="inbox panel" aria-label="Ticket inbox"><div class="inbox-heading"><h2>${inboxReadFilter === "approved" ? "Approved" : "Ticket inbox"}</h2><button class="text" data-action="refresh" aria-label="Refresh tickets">↻</button></div><div class="inbox-count"><span>${tickets.filter(x => !readTicketIds.has(x.id)).length} unread</span><span>${tickets.filter(x => ticketStatus(x).label === "Approved").length} approved</span></div><div class="inbox-filters" role="group" aria-label="Filter tickets"><button class="text" data-action="filter-all" aria-pressed="${inboxReadFilter === "all"}">All</button><button class="text" data-action="filter-unread" aria-pressed="${inboxReadFilter === "unread"}">Unread</button><button class="text" data-action="filter-approved" aria-pressed="${inboxReadFilter === "approved"}">Approved</button></div><div class="inbox-scroll" data-scroll-key="inbox">${visibleTickets.map(x => `<button class="ticket ${t?.id === x.id ? "chosen" : ""} ${readTicketIds.has(x.id) ? "is-read" : "is-unread"}" data-action="ticket" data-id="${esc(x.id)}" aria-current="${t?.id === x.id ? "true" : "false"}">${ticketMarker(x)}<span class="ticket-person"><span>${esc(x.customer_name)}</span><time title="${esc(date(x.submitted_at))}">${relativeDate(x.submitted_at)}</time></span><strong>${esc(x.subject)}</strong><p>${esc(x.complaint)}</p>${ticketAgentActivity(x)}</button>`).join("") || (inboxReadFilter === 'approved' ? '<p class="empty">No tickets are approved for human PR work yet.</p>' : inboxReadFilter === 'unread' ? '<p class="empty">All caught up. No unread tickets.</p>' : '<p class="empty">No tickets yet. New customer complaints will appear here.</p>')}</div></aside><div class="detail-column" data-scroll-key="ticket">${t ? ticketView(t) + ticketAgentPanel(t) : '<section class="workspace-empty"><span class="muted">2db / workspace</span><h2>Select a ticket to begin.</h2><p>Investigate, review the change, then verify the result.</p></section>'}</div></div></div>`;
}
function nextAction(t, run, p) {
  ...
  if (active) {
  ...
  } else if (p && ["Ready for Agent 2", "Awaiting engineer approval", "Proposal ready", "Changes requested"].includes(p.state)) {
    heading = "Ready for Agent 2"; message = `Agent 2 can independently check revision ${p.revision_number} and flag issues for your review.`;
    ...
    heading = "Approved for human PR work"; message = "This ticket is in the Approved queue. A human engineer can now create and review its pull request.";
...
function buildFindings(t, run, p) {
  ...
  return `<div class="findings"><div><h3>Observed behavior</h3><p>${run?.evidence?.length ? `${run.evidence.length} browser evidence capture${run.evidence.length === 1 ? "" : "s"} recorded. Open the evidence below to inspect the observations.` : "No browser evidence recorded yet."}</p></div><div><h3>Likely cause <span class="muted">· hypothesis</span></h3><p>${esc(report?.likelyCause || "No cause reported yet.")}</p></div><div><h3>Proposed change</h3><p>${esc(p?.explanation || "No change proposed yet.")}</p></div><div><h3>Remaining uncertainty</h3><p>${esc(report?.uncertainties || "Independent verification has not established a result yet.")}</p></div></div>${report?.sourceReferences?.length ? `<details><summary>Source references</summary>${report.sourceReferences.map(ref => `<p><code>${esc(ref)}</code></p>`).join("")}</details>` : ""}${researchCitationsView(p?.agentMetadata?.research)}${agentView(t)}`;
}
function reviewProposal(p) {
  const running = runningVerification(p), queued = p.state === "Approved";
  return `<section class="proposal-review"><div class="section-title"><h3>Proposed diff · revision ${p.revision_number}</h3>${badge(stageName(p.state))}</div><p>${esc(p.explanation)}</p><pre class="diff">${p.diff.split("\n").map(line => `<span class="${line.startsWith("+") ? "add" : line.startsWith("-") ? "remove" : ""}">${esc(line)}\n</span>`).join("")}</pre><details><summary>Revision details</summary>${hashLabel("CANDIDATE", p.candidate_revision)}${hashLabel("BASELINE", p.base_revision)}${hashLabel("REQUIREMENTS", p.requirements_hash)}${hashLabel("HARNESS", p.harness_hash)}${p.queueApproval ? `<p>Approved by ${esc(p.queueApproval.reviewer)} · ${date(p.queueApproval.created_at)}</p>` : ""}</details><p class="muted">Approval sends this Agent 2-reviewed revision to the Approved queue for a human engineer to create a pull request. It does not create, merge, or deploy a PR.</p><div class="actions">${reviewedByAgent2(p) && !queued ? `<button class="primary" data-action="approve" ${busy ? "disabled" : ""}>Approve</button>` : queued ? '<span class="approved-label">✓ In Approved queue</span>' : ""}${!queued ? `<button class="secondary" data-action="reject" ${running || busy || p.state === "Rejected" ? "disabled" : ""}>Deny</button>` : ""}</div></section>`;
}
function verificationFindings(p) {
  if (!p) return '<p class="empty">Waiting for Agent 1’s proposed change. Agent 2 checks it before human approval.</p>';
  return `${p.state === "Approved" ? '<p class="approved-label">✓ Human approved for PR preparation</p>' : '<p class="muted">Agent 2 reports findings for the human engineer; it cannot approve, merge, or deploy.</p>'}${p.runs?.length ? evidenceView(p, p.runs[0]) : '<p class="empty">No verification results yet.</p>'}<details><summary>Technical details · activity history</summary><ol class="timeline">${(p.activity || []).map(e => `<li><strong>${esc(e.event)}</strong><p>${esc(e.details)}</p><small>${esc(e.actor)} · ${date(e.at || e.created_at)}</small></li>`).join("")}</ol></details>`;
}
