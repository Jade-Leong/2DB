...
function render() {
  ...
  root.innerHTML = `<div class="shell"><div class="terminal-titlebar"><span class="traffic-lights" aria-hidden="true"><i></i><i></i><i></i></span><span>2db — engineer workspace</span><span class="titlebar-spacer"></span></div><div class="workspace"><header><a class="logo" href="/">2db</a><nav aria-label="Main navigation"><a href="${accountPage ? "/#agents" : "#agents"}">The agents</a><a href="${token ? "#review-workspace" : "/login"}">${token ? "Workspace" : "Log in"}</a>${!token ? '<a href="/signup">Sign up</a>' : ""}<a href="http://127.0.0.1:5173" target="_blank" rel="noreferrer">Loop Market ↗</a></nav><div class="engineer">${token ? '<span>you</span><button class="text" data-action="logout">Sign out</button>' : '<span class="muted">Local workspace</span>'}</div></header>${accountPage ? "" : `<section class="terminal-hero"><span class="breadcrumb">2db / engineer workspace</span><h1 data-type="hero-title">One builds.<br><span>One verifies.</span></h1><p data-type="hero-copy">Two separate agents for your engineering workflow.<br>One proposes the change. One verifies it. You decide what ships.</p><a class="hero-link" href="${token ? "#review-workspace" : "/signup"}">${token ? "Open your workspace" : "Get started"} ↓</a></section>${agentOverview()}`}<main id="review-workspace">${error ? `<div class="alert error" role="alert">${esc(error)}</div>` : ""}${notice ? `<div class="alert" role="status">${esc(notice)}</div>` : ""}
${
  !token
    ? accountView()
