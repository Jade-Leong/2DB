...
writeFileSync(path.join(publicRoot,'deployment-status.js'),`const banner=document.createElement('div');banner.className='hosted-preview-status';banner.textContent='Hosted test preview. Purchases are simulated. Live AI investigation is not yet validated; scripted demonstrations are labeled.';const link=document.createElement('a');link.href=location.pathname.startsWith('/control')?'/':'/control/';link.textContent=location.pathname.startsWith('/control')?'Loop Market':'2DB dashboard';banner.append(link);document.body.prepend(banner);`);
