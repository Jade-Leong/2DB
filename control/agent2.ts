...
type LiveResult = {
  ...
  actions: any[];
  usage: any;
  gates: { baselineBrowser: boolean; candidateBrowser: boolean; screenshots: boolean; regressionEvidence: boolean; paymentEvidence: boolean };
};
...
export async function agent2Status() {
  ...
  try {
    ...
    sdk = typeof agents.Agent === "function" && typeof agents.run === "function" && typeof agents.tool === "function";
  } catch {}
  const key = process.env.TWO_DB_OPENAI_API_KEY?.trim();
  const model = process.env.TWO_DB_AGENT_2_MODEL?.trim() || process.env.TWO_DB_AGENT_MODEL?.trim() || "";
  ...
  return {
    ...
    sdk: { ready: sdk, message: sdk ? "Official OpenAI Agents SDK installed" : "Run npm.cmd ci" },
    authentication: {
      ready: authenticated,
      message: !key
...
export class Agent2Service {
  ...
  constructor(
    ...
    public liveExecutor?: (runId: string, status: Agent2Status, signal: AbortSignal) => Promise<LiveResult>,
  ) {}

  async start(proposalId: string) {
    if (this.active) problem("A live Agent 2 verification is already running.", 409);
    const proposal = this.store.proposal(proposalId);
    this.store.approvalFor(proposal);
    const status = await this.statusCheck();
    if (status.state !== "Ready") problem("Agent 2 setup required: " + status.authentication.message + " " + status.isolation.message, 409);
    this.active = true;
  ...
  cancel(runId: string) {
    if (this.activeRunId !== runId) problem("Live Agent 2 run is not active.", 409);
    this.abort?.abort();
    const run = this.store.db.prepare("SELECT proposal_id FROM runs WHERE id=?").get(runId) as any;
    if (run) this.store.event(this.store.proposal(run.proposal_id).ticket_id, run.proposal_id, "Agent 2", "Cancellation requested", { runId });
    return { runId, state: "Cancelling" };
  }

  private async finish(runId: string, status: Agent2Status, signal: AbortSignal) {
    let row: any;
    ...
    if (signal.aborted) return this.recordFinal(row, "Inconclusive", { status: "cancelled", summary: "Engineer cancelled live Agent 2." });
    const scriptedEvidence = row.evidence ? JSON.parse(row.evidence) : null;
    if (row.state !== "Live Agent 2 running" || scriptedEvidence?.scriptedDecision !== "Verified awaiting engineer review") {
      this.store.event(proposal.ticket_id, proposal.id, "Agent 2", "Live exploration not started", {
        runId,
        reason: "Mandatory scripted checks did not establish a passing candidate.",
    ...
    try {
      this.store.approvalFor(proposal);
      const result = await (this.liveExecutor ?? this.explore.bind(this))(runId, status, signal);
      signal.throwIfAborted();
      const references = Array.isArray(result.assessment?.evidenceReferences)
        ? result.assessment.evidenceReferences
        : [];
      const validReferences = references.length > 0 && references.every((reference: string) =>
        result.actions.some((action) => action.artifact === reference) || /^D0[1-8]$/.test(reference),
      );
      const complete = result.assessment?.status === "resolved"
      ...
      const identityMatches = approval.id === row.approval_id && current.candidate_revision === row.candidate_revision && current.requirements_hash === row.requirements_hash && current.harness_hash === row.harness_hash;
      const finalState = complete && identityMatches ? "Verified awaiting engineer review" : result.assessment?.status === "unresolved" ? "Failed" : "Inconclusive";
      ...
      this.recordFinal(row, "Inconclusive", {
        ...
        summary: signal.aborted ? "Engineer cancelled live Agent 2." : "Live Agent 2 did not complete. Scripted evidence remains separately available.",
  ...
  private recordFinal(run: any, state: string, assessment: any) {
    ...
    const stillCurrent = proposal.last_run === run.id
    ...
    this.store.db.prepare("UPDATE runs SET state=?,finished_at=?,message=?,agent_assessment=? WHERE id=?")
      .run(state, now(), message, JSON.stringify(assessment), run.id);
    if (stillCurrent)
      this.store.db.prepare("UPDATE proposals SET state=?,updated_at=? WHERE id=?").run(state, now(), proposal.id);
    this.store.event(proposal.ticket_id, proposal.id, "Agent 2", state, { runId: run.id, message, verificationMode: "Live Agent 2" });
  }

  private async explore(runId: string, status: Agent2Status, signal: AbortSignal): Promise<LiveResult> {
    const runRow = this.store.db.prepare("SELECT * FROM runs WHERE id=?").get(runId) as any;
    const proposal = this.store.proposal(runRow.proposal_id);
    const ticket = this.store.db.prepare("SELECT * FROM tickets WHERE id=?").get(proposal.ticket_id) as any;
    const evidence = JSON.parse(runRow.evidence);
    const source = proposal.kind === "agent-generated"
      ? this.store.agentCandidate(proposal.id)
      : { base_root: this.store.fixtures.base.root, candidate_root: selectedFixture(this.store.fixtures, proposal.kind).root };
    ...
    if (revision(baselineRoot) !== runRow.base_revision || revision(candidateRoot) !== runRow.candidate_revision)
      throw new Error("Approved source identity mismatch before live exploration.");
    ...
    const gates = { baselineBrowser: false, candidateBrowser: false, screenshots: false, regressionEvidence: false, paymentEvidence: false };
    const timeout = setTimeout(() => this.abort?.abort(), 8 * 60_000);
    try {
      await Promise.all([
        baseline.start(baselineRoot, signal, (text) => writeFileSync(path.join(root, "baseline-build.log"), text, { flag: "a" })),
        candidate.start(candidateRoot, signal, (text) => writeFileSync(path.join(root, "candidate-build.log"), text, { flag: "a" })),
      ]);
      await Promise.all([
        baseline.browserAction({ action: "init", buyer: ticket.customer_id }),
        candidate.browserAction({ action: "init", buyer: ticket.customer_id }),
      ]);
      const { Agent, run, tool, setDefaultOpenAIKey, setTracingDisabled } = await import("@openai/agents");
      setDefaultOpenAIKey(process.env.TWO_DB_OPENAI_API_KEY!);
      setTracingDisabled(true);
      const browserTool = tool({
        name: "browser_action",
        description: "Use the restricted browser in the baseline or approved candidate. Inspect first, then choose normal customer UI actions. The tool cannot access other URLs or controller APIs.",
        parameters: z.object({
          environment: z.enum(["baseline", "candidate"]),
          action: z.enum(["open", "inspect", "click", "fill", "select", "screenshot", "responses"]),
          target: z.string().max(300).default(""),
          value: z.string().max(500).default(""),
        }),
        execute: async ({ environment, action, target, value }) => {
          if (actions.length >= 30) throw new Error("Live browser tool budget exhausted.");
          ...
          if (image.subarray(0, 8).toString("hex") !== "89504e470d0a1a0a")
            throw new Error("Trusted browser did not return a valid screenshot.");
          const artifact = `${environment}-${String(++screenshots).padStart(2, "0")}.png`;
          writeFileSync(path.join(artifactRoot, artifact), image, { flag: "wx" });
          const record = { at: now(), environment, action, target, artifact, url: observed.url, receipt: observed.receipt };
          actions.push(record);
          gates[environment === "baseline" ? "baselineBrowser" : "candidateBrowser"] = true;
      ...
      const checksTool = tool({
        ...
        description: "Read the trusted mandatory Playwright results for baseline and candidate. This is evidence, but you must also investigate with browser actions.",
        parameters: z.object({ environment: z.enum(["baseline", "candidate"]) }),
        execute: async ({ environment }) => {
          ...
          return { environment, status: assessment?.status, checks: assessment?.checks };
        },
      });
      const paymentTool = tool({
        name: "read_recorded_payment",
        description: "Read the narrow trusted payment observation captured by check D01. Arbitrary SQL is unavailable.",
        parameters: z.object({ environment: z.enum(["baseline", "candidate"]) }),
        execute: async ({ environment }) => {
          paymentReads.add(environment);
          gates.paymentEvidence = paymentReads.size === 2;
          const check = evidence[environment]?.required?.assessment?.checks?.find((item: any) => item.id === "D01");
          return { environment, status: check?.status, expected: check?.expected, observed: check?.observed };
        },
      });
