...
function agentOverview() {
  const status = token ? agentStatus?.state || "Checking setup" : "Log in to check agent status";
...
function localAccess() {
  return `<details class="local-access" ${accountConfig?.enabled ? "" : "open"}><summary>Local engineer access</summary><p>Use the existing local engineer key to open this workspace.</p><form id="login"><label>Local engineer key<input name="key" type="password" autocomplete="off" required></label><button ${busy ? "disabled" : ""}>Open engineer session →</button></form><p class="muted">Get the key with <code>npm run control:engineer</code> in a separate terminal.</p></details>`;
}

function accountView() {
  const signup = accountMode === "signup";
  return `<section class="login panel" aria-label="Account access"><div class="account-tabs" role="tablist" aria-label="Account access"><button id="login-tab" type="button" role="tab" aria-controls="account-panel" aria-selected="${!signup}" data-account-mode="login">Log in</button><button id="signup-tab" type="button" role="tab" aria-controls="account-panel" aria-selected="${signup}" data-account-mode="signup">Sign up</button></div><div id="account-panel" role="tabpanel" aria-labelledby="${signup ? "signup" : "login"}-tab"><span class="eyebrow">you</span><h2>${signup ? "Join the workspace." : "Back to building."}</h2><p>${signup ? "Create an account to access the shared tickets, proposals, and verification results." : "Log in to review tickets and work with your agents."}</p>${accountMessage ? `<p class="auth-message" role="status">${esc(accountMessage)}</p>` : ""}${accountError ? `<p class="auth-message error" role="alert">${esc(accountError)}</p>` : ""}<form id="account-form"><label>Email<input name="email" type="email" autocomplete="email" maxlength="254" required></label><label>Password<input name="password" type="password" autocomplete="${signup ? "new-password" : "current-password"}" minlength="${signup ? 12 : 1}" maxlength="128" ${signup ? 'aria-describedby="password-hint"' : ""} required></label>${signup ? '<p id="password-hint" class="muted">Use at least 12 characters. You’ll confirm your email before logging in.</p>' : ""}<button ${accountBusy || !accountConfig?.enabled ? "disabled" : ""}>${accountBusy ? "Please wait…" : signup ? "Create account →" : "Log in →"}</button></form>${accountConfig?.enabled ? '<p class="muted">Every account joins the same engineering workspace.</p>' : `<p class="muted" role="status">${accountConfig === null ? "Checking account access…" : "Email login is not configured. Use local engineer access below."}</p>`}</div>${localAccess()}</section>`;
}
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (/access_token=|refresh_token=|error_description=/.test(location.hash)) {
    ...
    accountMessage = params.has("error_description") ? "The confirmation link could not be completed. Try logging in or request a new account confirmation." : "Email confirmation received. Log in to continue.";
