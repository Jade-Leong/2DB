...
The visual implementation lives in `control/web/`: `terminal.css` layers the Terminal theme over the existing components; `terminal-motion.js` adds one-time scroll typing to introduction copy; `account.js` supplies the login/sign-up views. All original ticket and proposal actions continue to use the existing controller API. The header uses the provided `logo-reference.png` asset.
