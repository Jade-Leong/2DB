...
export const scenarios = ['idle', 'running', 'proposal-ready', 'approved', 'verification-running', 'failed', 'completed', 'inconclusive'];
export function fixtureData(scenario = 'idle') {
  ...
  const result = ({failed:'Failed',completed:'Verified awaiting engineer review',inconclusive:'Inconclusive'})[scenario];
  ...
  const p = hasProposal ? { id:'fixture-proposal',ticket_id:t.id,ticket:t,kind:'agent-generated',author:'Agent 1',state: result || (verifying ? 'Live Agent 2 running' : approved ? 'Approved for testing':'Awaiting engineer approval'),revision_number:1,candidate_revision:'c'.repeat(64),base_revision:'b'.repeat(64),requirements_hash:'r'.repeat(64),harness_hash:'h'.repeat(64),current_approval:approved?'fixture-approval':null,last_run:verifying||result?'fixture-verification':null,approvals:approved?[{id:'fixture-approval',revision:'c'.repeat(64),revision_number:1,reviewer:'Fixture engineer',created_at:new Date().toISOString()}]:[],explanation:'Pass the discounted order total into the payment request so the charge matches checkout.',diff:'--- a/server/payments.ts\n+++ b/server/payments.ts\n- amount: subtotalCents,\n+ amount: order.totalCents,',agentMetadata:{conclusion:{likelyCause:'The payment handler uses the subtotal before the discount is applied.',uncertainties:'Other payment methods have not been evaluated.',sourceReferences:['server/payments.ts']},expected:'The charged total should match checkout.'},requirements:[{id:'total-match',title:'Charged total matches checkout',expected:3840}],activity:[],runs:[]} : null;
  if (p && (verifying || result)) p.runs=[{id:'fixture-verification',state:p.state,revision_number:1,candidate_revision:p.candidate_revision,approval_id:p.current_approval,verification_mode:'live-agent-2',started_at:new Date(Date.now()-30000).toISOString(),finished_at:verifying?null:new Date().toISOString(),message:verifying?'Comparing the baseline and candidate checkout results.':result==='Failed'?'The candidate still charges the full amount.':result==='Inconclusive'?'The browser session ended before all checks completed.':'The checkout amount and payment match in the candidate.',evidence:verifying?null:Object.fromEntries(['baseline','candidate'].map(env=>[env,{typecheck:{exitCode:0},build:{exitCode:0},required:{exitCode:env==='candidate'&&scenario==='completed'?0:1,assessment:{checks:[{id:'total-match',status:env==='candidate'&&scenario==='completed'?'passed':'failed',observed:env==='candidate'&&scenario==='completed'?3840:4800}]},report:{tests:[]}}}]))}];
